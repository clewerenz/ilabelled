...
test_that("i_recode - recode i_labelled var - no error", {
  expect_no_error(i_recode(x$V1, "AB" = 1 ~ c("A", "B"), "CD" = 2 ~ c("C", "D")))
})
test_that("i_recode - recode i_labelled var - no error", {
  expect_no_error(i_recode(x$V1, "AB" = 1 ~ c("A", "B"), "CD" = 2 ~ c("C", "D")))
})
test_that("i_recode - recode factor var - no error", {
  expect_no_error(i_recode(x$V2, "AB" = 1 ~ c("A", "B"), "CD" = 2 ~ c("C", "D")))
})
test_that("i_recode - recode character var - no error", {
  expect_no_error(i_recode(x$V3, "AB" = 1 ~ c("A", "B"), "CD" = 2 ~ c("C", "D")))
})
test_that("i_recode - recode numeric var - no error", {
  expect_no_error(i_recode(x$V4, "AB" = 1 ~ c(1, 2), "CD" = 2 ~ c(3, 4)))
})

test_that("i_recode - apply variable label - no error", {
  expect_no_error(
    res <- i_recode(x$V1, "AB" = 1 ~ c("A", "B"), "CD" = 2 ~ c("C", "D"), label = "myRecoding")
  )
  expect_equal(attr(res, "label"), "myRecoding")
})
test_that("i_recode - apply na_values - no error", {
  expect_no_error(
    res <- i_recode(x$V4, "AB" = 1 ~ c(1, 2), "CD" = 2 ~ c(3, 4), "X" = -9 ~ -9, na_values = -9)
  )
...
test_that("wrong data class data.frame", {
  expect_error(i_recode(iris, "AB" = 1 ~ c(1, 2), "CD" = 2 ~ c(3, 4)))
})
